      ...
      const confirmEdit = async () => {
        ...
        try {
          ...
          if (outcome.mode === "fork") {
            const forkResult = await sessions.fork({ sessionId, atSeq: outcome.boundarySeq, increaseTitle: true });
            if (!forkResult || forkResult.ok !== true) {
              const detail = forkResult && forkResult.error;
              throw new Error((detail && (detail.message || detail)) || "分支出错");
            }
            childId = forkResult.value.sessionId;
          } else if (outcome.mode === "fresh") {
            childId = await workspaces.connectWorkspace(outcome.workspaceId);
            if (childId && typeof childId === "object" && childId.ok !== undefined) {
              if (childId.ok !== true) {
                const detail = childId.error;
                throw new Error((detail && (detail.message || detail)) || "新建会话失败");
              }
              childId = childId.value.sessionId;
            }
            if (typeof childId !== "string") throw new Error("新建会话失败");
          } else {
